...
function wpgp_ajax_govr_create_contrib() {
  ...
  wpgp_db_create_contrib($_POST['data']['title'],
                         $_POST['data']['theme_id'],
                         $_POST['data']['content'],
                         $current_user->ID,
                         $_POST['data']['part']);
...
function wpgp_ajax_govr_delete_contrib() {
    $org = wpgp_db_get_contrib($_POST['data']['id']);
    wpgp_db_delete_contrib($id, !!$org['created_by_moderation']);
}
...
function wpgp_ajax_govr_update_contrib() {
    ...
    case 'status':
    case 'theme_id':
      wpgp_db_update_contrib($_POST['data']['id'],
                             $_POST['data']['field'],
                             $_POST['data']['value']);
      break;
    case 'parent':
      ...
      if ($_POST['data']['value'] != "0") {
        $parent = wpgp_db_get_contrib($_POST['data']['value']);
        if ($parent == null) {
          die("not-found");
        }
      }
    ...
    case 'part':
      ...
      if ($_POST['data']['value'] != "0") {
        ...
        if (empty($_POST['data']['value'])) {
          foreach (wpgp_contrib_get_parents($org) as $parent) {
            wpgp_contrib_remove_part($parent, $org);
          }
          die('ok');
        }
        if (wpgp_contrib_insert_parts($org,$_POST['data']['value'])) {
          die('ok');
        } else {
          die('not-found');
        }
